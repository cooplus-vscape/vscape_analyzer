...
def get_oob_offsets(counterfeit_classname,vmethodid):
    ...
    for it in ooblist:
        ...
        accesstype = it["accesstype"].lower()
        if accesstype.find("w")!=-1:
...
def is_offset_toward_ptr(aunit):
    classname = aunit[1]
    offset = aunit[2]
    # layout_cl.find()
    fi = {
...
# sys.stdout = ToFileStdOut("./w_ptr_cs.out")
                ...
                if len(aunit)==3:
                    ...
                    for offset in offsetlist:
                        if offset !=-1:
                            ...
                            try:
                                vname = is_offset_toward_ptr(taskunit)
                                if vname:
...
print (count_write)
print(count_write_ptr)
# for result in resultlist:
#     if is_offset_toward_ptr(result):
